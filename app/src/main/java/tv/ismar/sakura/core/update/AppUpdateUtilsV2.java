...
public class AppUpdateUtilsV2 extends Handler {
    ...
    public void checkAppUpdate() {
        ...
        String manu = "sharp";
        String model = Build.PRODUCT.replace(" ", "_");


        OkHttpClientManager.getInstance().resetAdapter_APP_UPDATE.create(SakuraClientAPI.CheckAppUpdate.class).doRequest(sn, manu, app, model, location, ver).enqueue(new retrofit2.Callback<VersionInfoV2Entity>() {
            @Override
    ...
    private void updateProcess(VersionInfoV2Entity versionInfoV2Entity) {
        ...
        if (packageInfo.versionCode < Integer.parseInt(versionInfoV2Entity.getApplication().getVersion())) {
            if (apkFile.exists()) {
                ...
                String localMd5Code = DeviceUtils.getMd5ByFile(apkFile);
