...
class CordIrisRpcClient(object):
    ...
    def compensate(self, user_pk, package_pk=None, item_pk=None,ilog=None):
        self.response = None;
        ...
        self.channel.basic_publish(exchange='exchange.cord_iris',
                ...
                body=json.dumps(dict(method='compensate', user_pk=user_pk,package_pk=package_pk, item_pk=item_pk)))
...
class PointlogAdmin(admin.ModelAdmin):
    list_display = (point,'user_agent',speeds , 'description',clip, 'phone', 'mail', 'ip', 'location','isp','create_date','status','update_date')
    list_editable = ('status',)
    search_fields = ('user_agent','phone','mail','ip','description','location',)
    radio_fields = {'status':admin.HORIZONTAL}
    readonly_fields = (point,'user_agent','speeds' , 'description', 'phone', 'mail', 'ip', 'location','isp','create_date','update_date',clip,)
...
class IndemnityAdmin(admin.ModelAdmin):
    list_display = ('customer',packages,items,'matter','update_date',)
...
class IndemnityLogAdmin(admin.ModelAdmin):
    list_display = ('customer','package','item','status','error_desc','error_code','correlation_id','create_date','update_date',)
    readonly_fields =('customer','package','item','correlation_id','error_desc','error_code','create_date','update_date','status',)
    search_fields = ('customer__name','package__title','item__title')
